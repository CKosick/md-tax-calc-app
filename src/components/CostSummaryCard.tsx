      ...
      <div className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-xl backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/90 sm:p-8">
        ...
        <div className="my-6 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-6 text-white shadow-lg shadow-indigo-950/20 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950">
          ...
          <p className="mt-2 text-xs text-slate-300">
            Excise Tax + Title Certificate + Tags & Registration {breakdown.lienFilingFee > 0 ? '+ Lien Fee' : ''}
          </p>
        </div>
              ...
              <div className="pr-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-200">
                    Vehicle Excise Tax ({(rule.exciseTaxRate * 100).toFixed(1)}%)
                  </span>
                </div>
                <p className="mt-0.5 pl-6.5 text-xs text-slate-500 dark:text-slate-400">
                  {breakdown.minTaxApplied
                    ? `Assessed at statutory minimum floor of ${formatCurrency(rule.minExciseTax ?? 41.60)}`
                    : `6.5% on purchase price of ${formatCurrency(purchasePrice)}`}
                </p>
              </div>
        ...
        {/* Informational VEIP Section (not part of total per §1) */}
        ...
        <div className="mt-6 space-y-2">
          {breakdown.bookValueApplies && (
            <div className="rounded-xl border border-amber-200/90 bg-amber-50/90 p-3.5 text-xs text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-200">
              <div className="flex items-center gap-1.5 font-bold text-amber-800 dark:text-amber-300">
                ...
                <span>MVA Book Value Assessment Rule (≤ 7 Years Old)</span>
              </div>
              <p className="mt-1 leading-relaxed">
                Because this vehicle is 7 years old or newer ({breakdown.vehicleAge} years old), MVA may assess the 6.5% tax on NADA book value instead of your purchase price unless accompanied by a notarized Maryland Bill of Sale (VR-181).
              </p>
            </div>
          )}
          ...
          {breakdown.tradeInIgnored && (
            <div className="rounded-xl border border-indigo-200/90 bg-indigo-50/90 p-3.5 text-xs text-indigo-900 dark:border-indigo-900/50 dark:bg-indigo-950/40 dark:text-indigo-200">
              <span className="font-bold">Trade-in Note:</span> Maryland does not permit trade-in deductions on private sales. Your excise tax is calculated on the full agreed vehicle price.
            </div>
          )}
        </div>
      </div>
